...
def load_filter_dat(path):
    """
    Load a filter profile .dat from SVO

    Params
    path : str
        path to .dat file from svo
    wavelength_unit : {"Angstrom", "nm"}
        Unit of wavelength in file (typically angstrom)
    note that this need to have code that distinguishes between nm and angstrom etc.
    Returns
    lam : wavelength in nm
    trans : normalised tranmission (0-1)
    """
...
def bandpass_irradiance(lam_nm_filter, T_filter,
                        lam_nm_sun, E_sun_Wm2_per_nm,
                        return_norm=False):
    """
    ...
    Params
    lam_nm_filter: array 
    wavelength grid of filter transmission curve nm
    T_filter: array
    Filter transmission 0-1
    lam_nm_sun: array
    wavelength grid of solar spectrum nm
    E_sun_Wm2_per_nm: array
    Solar spectral irradiance at TOA on lam_nm_sun W/m^2/nm
    return_norm:
    If true return integral T dlamb and effective wavelength 

    Returns
    I_band: float in W/m^-2
    In band irradiance after filter+reflectance weighting
    Multiply by geometric/BRDF term later
    """
    ...
    I_band = float(np.trapz(integrand, lam_nm_filter)) #lam_f in nm, E in W m^-2 nm^-1 → W m^-2
    ...
    den_piv = float(np.trapz(T_filter / np.clip(lam_nm_filter, 1e-12, None), lam_nm_filter))
    lambda_p = np.sqrt(num_piv / den_piv) if den_piv > 0 else np.nan #pivot lambda for AB mag conv
...
def sample_cosine_hemisphere(n_samples, rng=None):
    ...
    u1 = rng.random(n_samples); u2 = rng.random(n_samples)
    r  = np.sqrt(u1); phi = 2*np.pi*u2
    x = r*np.cos(phi); y = r*np.sin(phi); z = np.sqrt(1.0 - u1)
    return x, y, z  #local (+Z = surface normal)

def _frame_from_normal(n):
    nx, ny, nz = n
    n = np.array(n, float); n /= (np.linalg.norm(n)+1e-16)
    up = np.array([0,0,1.0]) if abs(n[2]) < 0.999 else np.array([1.0,0,0])
    t  = np.cross(up, n); t /= (np.linalg.norm(t)+1e-16)
    b  = np.cross(n, t)
    return t, b, n
...
def hemispherical_reflectance(brdf_fn, wi, n, lam=None, nk=None, n_samples=4096, seed=42):
...
def SPECTRAL_ENERGY(base_brdf, reflectance_df,
    ...
    R_model = np.array([
        hemispherical_reflectance(base_brdf, wi, n, lam=L,
                                  n_samples=rho_samples, seed=rho_seed if rho_seed is not None else 42)
        for L in lam_tab
    ], dtype=float)
