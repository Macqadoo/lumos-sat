...
class EarthMesh:
    ...
    def __init__(self, angles_off_plane, angles_on_plane):
        ...
        self.angles_off_plane, self.angles_on_plane = np.meshgrid(
        ...
        self.z = lumos.constants.EARTH_RADIUS / np.sqrt(
            1 + np.tan(self.angles_on_plane) ** 2 + np.tan(self.angles_off_plane) ** 2
        )
    ...
    def calculate_intensity(
        ...
        for i in range(self.shape[0]):
            for j in range(self.shape[1]):
                ...
                self.intensities[i, j] = lumos.calculator.get_intensity_satellite_frame(
                    surfaces,
                    self.sat_height,
