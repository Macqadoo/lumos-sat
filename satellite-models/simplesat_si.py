...
# A simple satellite model using Si solar cells
...
# GPS solar cells
GPS_DHR_df = pd.read_csv(os.path.join(material_dir, "GPS-cell-dhr.csv"), comment="#")
GPS_lam = GPS_DHR_df.iloc[:, 0].to_numpy(dtype=float)
GPS_rho = GPS_DHR_df.iloc[:, 1].to_numpy(dtype=float)
GPS_DHR_df = pd.DataFrame({"lambda_nm": GPS_lam, "R": GPS_rho})
...
BRDFS = {
    ...
    # Si solar cell: low diffuse, moderate specular
    "solar_gps": diffuse_spectral_energy_function(
        panel_BRDF,
        GPS_DHR_df,
        normalize_incident_deg=8.0,
        rho_samples=30000,
...
# Solar wings + Dishes
s_wing_port = Surface(A_wing, n_wing_port, brdf("solar_gps"))
s_wing_star = Surface(A_wing, n_wing_starboard, brdf("solar_gps"))
